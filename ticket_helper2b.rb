...
class Recurring_text_generation
  ...
  def task_escalation(c_email,c_name,c_unit,sr,t_title,c_tel,desc)
    ...
    puts "==============================="
    puts "escalated task for #{c_name} #{c_unit}"
    puts "==============================="
    puts "notified #{c_unit} of escalation"
    puts "copied task request to #{c_unit}"
  ...
  def generic_request_access_to_server(ticket_num,servers)
    ...
    puts "\n\n"
    puts "requested access"
    puts "sent email"
    puts "changed status to waiting on customer"
